            ...
            if Patcher.apply_patch(self.repo_path, suggestion.diff):
                ...
                if create_pr:
                    ...
                    if gh.push_changes(branch_name):
                        # Construct an attractive PR body
                        pr_body = f"""## 🤖 Automated Self-Healing Fix

### 🚨 Issue Detected
**Test Case:** `{target.test_class}.{target.test_name}`
**Failure Type:** `{target.failure_type}`

### 🛠 Fix Applied
{suggestion.explanation}

### 🎯 Confidence Score
`{suggestion.confidence * 100:.1f}%` - AI-generated fix confidence
...
---
*This PR was automatically generated by the **Self-Healing Agent**.*
*Powered by **Google Gemini 2.0 Flash** ✨*
"""
                        pr_url = gh.create_pr(
                            branch_name=branch_name,
                            title=f"🤖 Fix: {target.test_name}",
                            body=pr_body
                        )
                        if pr_url:
