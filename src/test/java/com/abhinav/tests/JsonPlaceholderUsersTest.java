...
public class JsonPlaceholderUsersTest {
  ...
  public void testUsersApiWithDtoComparison() {
    ...
    JsonPlaceholderUserDto[] actualUsers =
        com.abhinav.framework.controller.JsonPlaceholderController.getUsers();
    ...
    System.out.println(
        "Comparing expected user (ID: 1) with the first user from the API response...");
    DtoComparisonUtil.compareAndAssert(expectedFirstUser, actualUsers[0]);
    System.out.println(
        "DTO Comparison successful. The actual user matches the expected user exactly.");
  }
}
