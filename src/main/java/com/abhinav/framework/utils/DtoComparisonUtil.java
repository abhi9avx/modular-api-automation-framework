 ...
 * 
 * DTOComparisonUtil compares two DTOs (expected vs actual) and reports
 * differences.
 * 
 * What it does (simple)
 * - Takes two objects (expected and actual)
 * - Compares them field by field
 * - Finds differences
 * - Throws an error with details if they don't match
 * 
 * Real-world analogy
 * Like comparing two forms:
 * - Expected: What you think the API should return
 * - Actual: What the API actually returned
 * - Result: Lists all fields that don't match
 * 
 * How it's used
 * // Create expected response
 * GetConversationsResponseDTO expectedResponse =
 * GetConversationsResponseDTO.createDefault();
 * // Get actual response from API
 * GetConversationsResponseDTO actualResponse =
 * controller.getConversations(headers, 200);
 * // Compare them - throws error if different
 * DtoComparisonUtil.compareAndAssert(expectedResponse, actualResponse);
 * 
 * What happens inside
 * - Converts both DTOs to JSON trees
 * - Compares each field recursively
 * - Tracks differences (missing fields, wrong values, etc.)
 * - If differences found → throws error with details
 * - If no differences → test passes
 */
public class DtoComparisonUtil {
     ...
     * @param ignoredFields Fields/paths to ignore (e.g. "id", "data.createdAt",
     *                      "users[0]").
     */
    ...
    /**
     * Compares two DTOs and logs the differences (useful for debugging, without
     * failing).
     *
     * @param expected      The expected DTO or object.
     * @param actual        The actual DTO or object.
    ...
    /**
     * Computes the list of string differences between two objects across their
     * fields.
     */
    private static List<String> getDifferences(Object expected, Object actual, String... ignoredFields) {
        List<String> diffs = new ArrayList<>();
        List<String> ignoredList = ignoredFields != null ? Arrays.asList(ignoredFields) : new ArrayList<>();
     ...
     */
    private static void compareNodes(String path, JsonNode expected, JsonNode actual, List<String> ignored,
            List<String> diffs) {
        ...
        if (expected.getNodeType() != actual.getNodeType()) {
            diffs.add(String.format("Path '%s': Type mismatch. Expected %s but was %s",
        ...
        if (expected.isObject()) {
            ...
            while (actualIter.hasNext()) {
                ...
                if (!expected.has(fieldName) && !ignored.contains(childPath)) {
                    diffs.add(
                            String.format("Path '%s': Extra field present in actual but not in expected.", childPath));
                }
            }
        } else if (expected.isArray()) {
            if (expected.size() != actual.size()) {
                diffs.add(String.format("Path '%s': Array size mismatch. Expected %d items but got %d items.",
            ...
            if (!expected.equals(actual)) {
                diffs.add(String.format("Path '%s': Expected value '%s' but was '%s'",
